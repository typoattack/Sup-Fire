...
public class foodTrigger_4_2 : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (countDown < 0)
        {
            ...
            if (topOrBottom >= 0f)
                newChild = Instantiate(randomChild, new Vector3(0f, 4.5f, -0.5f), new Quaternion(0f, 0f, 0f, 0f)) as GameObject;
            if (topOrBottom < 0f)
                newChild = Instantiate(randomChild, new Vector3(0f, -6.3f, -0.5f), new Quaternion(0f, 0f, 0f, 0f)) as GameObject;
