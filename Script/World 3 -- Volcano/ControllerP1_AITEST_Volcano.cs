...
public class ControllerP1_AITEST_Volcano : MonoBehaviour
{
    ...
    void GetLavalPos(float x)
    {
        lavalpos = x;
    }
    void GetLavalTime(float x)
    {
        lavalTime = x;
    }
    ...
    {
        int k = 0;
        if (Time.time - lavalTime < 2&&lavalTime!=0)
            k = 0;//laval time
        else
            k = 1;//No laval time
        if (Time.time - BulletPosLastTime < 2 && k == 1)
        {
        ...
        else if (Time.time - lavalTime < 2 && k == 0)
        {
            if (transform.position.x <= -6.8f)
                Movespeed = 1;
            else if (transform.position.x >= -2)
                Movespeed = -1;
            else if (lavalpos - transform.position.x <= 2 && lavalpos - transform.position.x > 0.5)
                Movespeed = -1;//move left 
            else if (lavalpos - transform.position.x <= 1 && lavalpos - transform.position.x > -2)
                Movespeed = 1; ;//move right
